...
public class televisionPanel : BasePanel
{
    ...
    public void ControlCheckRuleButton(bool IsShow)
    {
        controlDic["CheckRuleButton"].gameObject.SetActive(IsShow);
    }
    ...
    public override void ClickButton(string controlName)
    {
        ...
        switch (controlName)
        {
            ...
            case "CheckRuleButton":

                UImanager.Instance.ShowPanel<WarnPanel>().SetText(
                "花色规则",  // 面板标题（不变）
                         // 左侧：玩家规则 | 右侧：敌人规则（用\t和|分隔，确保左右对齐）
                "【玩家牌规则】\t\t\t\t| 【敌人牌规则】\n" +
                "• 两张黑桃：玩家结果乘二\t\t| • 黑桃：敌人结果乘二\n" +
                "• 两张红桃：敌人结果减百分之十\t| • 红桃：玩家结果减百分之十\n" +
                "• 带有梅花的牌：该牌数据变为负数\t| • 梅花：玩家第二张牌变为负数\n" +
                "• 两张方片：恢复1生命值\t\t| • 方片：失败额外扣1血"
