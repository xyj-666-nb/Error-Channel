...
public class StartPanel : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        ExitGameButton.interactable = true;
        StartGameButton.onClick.AddListener(() => { timeline.Play(); ExitGameButton.interactable = false; ExitGameButton.interactable = false;  });//启动时间线 
        ExitGameButton.onClick.AddListener(() => {  Application.Quit(); });//退出游戏
    ...
    private void SetRenderPipeline()
    {
        ...
        {
            GraphicsSettings.renderPipelineAsset = targetRenderPipeline;
            QualitySettings.renderPipeline = targetRenderPipeline;
            Debug.Log($"已设置渲染管线: {targetRenderPipeline.name}");
        }
        else
        {
            Debug.LogWarning("未指定渲染管线资源，使用默认渲染管线");
        }
    }
    ...
    private IEnumerator AsyncLoadScene(string sceneName)
    {
        ...
        // 场景切换完成后，将渲染管线置空
        SetRenderPipelineToNull();
    }

    // 将渲染管线置空（使用内置渲染管线）
    private void SetRenderPipelineToNull()
    {
        GraphicsSettings.renderPipelineAsset = null;
        QualitySettings.renderPipeline = null;
        Debug.Log("渲染管线已切换为内置渲染管线");
    }
