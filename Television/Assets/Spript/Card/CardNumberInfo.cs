...
public class CardNumberInfo : MonoBehaviour
{
    ...
    private int GetPlayerRandomNum(int min, int max, float highRate = 0.3f)
    {
        // 30%概率取较高区间的值，提高玩家高数值概率
        if (Random.value < highRate)
        {
            int mid = (min + max) / 2;
            return Random.Range(mid, max + 1); // 取中高区间
        }
        return Random.Range(min, max + 1); // 正常区间
    }

    // 辅助方法：生成波动更大的随机数（敌人用）
    private int GetEnemyRandomNum(int min, int max, float lowRate = 0.4f)
    {
        // 40%概率取较低区间的值，增加玩家获胜机会
        if (Random.value < lowRate)
        {
            int mid = (min + max) / 2;
            return Random.Range(min, mid + 1); // 取中低区间
        }
        return Random.Range(min, max + 1); // 正常区间
    }
    ...
    private void InitLevel1Data()
    {
        ...
        // 玩家数据：范围提高（8-22），波动更小，高数值概率更高
        for (int i = 0; i < dataCount; i++)
        {
            int randomNum = GetPlayerRandomNum(8, 22); // 玩家基础值高于原来的5-21
        ...
        // 敌人数据：范围降低（1-25），增加低数值概率
        for (int i = 0; i < dataCount; i++)
        {
            int randomNum = GetEnemyRandomNum(1, 25); // 敌人上限从31降到25
    ...
    private void InitLevel2Data()
    {
        ...
        // 玩家数据：范围提高（100-250），高数值概率更高
        for (int i = 0; i < dataCount; i++)
        {
            int randomNum = GetPlayerRandomNum(100, 250); // 玩家基础值高于原来的80-230
            playerData.Add(new NumberInfo
            {
                Number = randomNum,
                EquationString = randomNum.ToString(),
        ...
        // 敌人数据：范围降低（40-180），增加低数值概率
        for (int i = 0; i < dataCount; i++)
        {
            int randomNum = GetEnemyRandomNum(40, 180); // 敌人上限从201降到180
            enemyData.Add(new NumberInfo
            {
                Number = randomNum,
                EquationString = randomNum.ToString(),
    ...
    private void InitLevel3Data()
    {
        ...
        int playerDataCount = 30;
        List<NumberInfo> playerData = new List<NumberInfo>();
        for (int i = 0; i < playerDataCount; i++)
        {
            int randomNum = Random.Range(70, 150); // [80,200]闭区间
            playerData.Add(new NumberInfo
            {
                Number = randomNum,
                EquationString = randomNum.ToString(),
        ...
        List<(string equation, int result)> level3EnemyRawData = new List<(string, int)>()
{
    ...
    {
        // 1. 玩家数据调整：降低数值上限，缩小波动范围（原130-291 → 150-260）
        int playerDataCount = 30;
        List<NumberInfo> playerData = new List<NumberInfo>();
        for (int i = 0; i < playerDataCount; i++)
        {
            // 调整点1：数值范围从[130,291]降至[150,260]，平均数值降低约20点
            // 调整点2：增加高数值概率（40%概率取190-260区间），提升玩家获得优势牌的机会
            int randomNum;
            if (Random.value < 0.4f)
            {
                randomNum = Random.Range(160, 120); // 中高数值区间（占比40%）
            }
            else
            {
                randomNum = Random.Range(130, 150); // 基础数值区间（占比60%）
            }
            ...
            playerData.Add(new NumberInfo
            {
                Number = randomNum,
                EquationString = randomNum.ToString(),
                // 调整点3：提高玩家"黑桃""红桃"等优势花色概率（原GetRandomDesignMode基础上加权）
                CardMode = GetPlayerFriendlyDesignMode()
            });
        }

        // 2. 敌人数据调整：筛选低数值条目，降低整体强度
        List<(string equation, int result)> level4EnemyRawData = new List<(string, int)>()
{
    ...
    ("492 - 197", 295), ("68 × 6", 408), ("1287 ÷ 3", 429), ("289 + 167", 456), ("512 - 234", 278)
        ...
        List<NumberInfo> enemyData = new List<NumberInfo>();
        // 调整点4：优先筛选敌人低数值条目（≤300），再随机挑选20条
        var lowPowerEnemyData = level4EnemyRawData.Where(data => data.result <= 300).ToList();
        HashSet<int> selectedIndices = new HashSet<int>();

        // 若低数值条目足够（当前有18条≤300），先选完低数值，剩余用常规数值
        int lowDataCount = Mathf.Min(20, lowPowerEnemyData.Count);
        while (selectedIndices.Count < lowDataCount)
        {
            int randomIdx = Random.Range(0, lowPowerEnemyData.Count);
            if (selectedIndices.Add(randomIdx))
            {
                var data = lowPowerEnemyData[randomIdx];
                enemyData.Add(new NumberInfo
                {
                    Number = data.result,
                    EquationString = data.equation,
                    // 调整点5：降低敌人优势花色（黑桃、红桃）概率，减少对玩家的压制
                    CardMode = GetEnemyWeakDesignMode()
                });
            }
        }

        // 若还需补充条目（当前低数值仅18条，需补2条），从原列表选≤350的数值
        while (selectedIndices.Count < 20)
        {
            int randomIdx = Random.Range(0, level4EnemyRawData.Count);
            var data = level4EnemyRawData[randomIdx];
            if (data.result <= 350 && selectedIndices.Add(randomIdx))
        {
                enemyData.Add(new NumberInfo
                {
                    Number = data.result,
    ...
    // 新增：玩家友好型花色生成（提高优势花色概率）
    private CardDesignMode GetPlayerFriendlyDesignMode()
    {
        float rand = Random.value;
        // 调整花色概率：黑桃（双牌乘2）25%、红桃（双牌减敌人10%）25%、方片（双牌回血）25%、梅花（单牌变负）25%
        if (rand < 0.25f) return CardDesignMode.Spades;
        else if (rand < 0.5f) return CardDesignMode.RedHeart;
        else if (rand < 0.75f) return CardDesignMode.RedAngularShape;
        else return CardDesignMode.BlackPlumBlossom;
    }

    // 新增：敌人弱势花色生成（降低优势花色概率）
    private CardDesignMode GetEnemyWeakDesignMode()
    {
        float rand = Random.value;
        // 调整花色概率：梅花（让玩家第二张变负）35%、方片（玩家失败多扣血）30%、红桃（玩家减10%）20%、黑桃（已处理乘2，概率最低）15%
        if (rand < 0.35f) return CardDesignMode.BlackPlumBlossom;
        else if (rand < 0.65f) return CardDesignMode.RedAngularShape;
        else if (rand < 0.85f) return CardDesignMode.RedHeart;
        else return CardDesignMode.Spades;
    }
    ...
    public void SetCardInfo(Card card)
    {
        ...
        card. MyNumber.EquationText.color = (card.MyDesignMode == CardDesignMode.RedHeart || card.MyDesignMode == CardDesignMode.RedAngularShape) ? Color.black : new Color(255 / 255f, 124 / 255f, 62 / 255f);
    }
